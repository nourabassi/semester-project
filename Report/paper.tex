...
all the data analysis tools we employ.
    ...
    \begin{figure}
        \begin{center}\adjustimage{max size={\linewidth}{0.4\paperheight}}{paper_files/paper_7_0.png}\end{center}
...
\textbf{To glue back substrings} we exploit the fact that that APA
...
publishing venue follows.
    ...
    \begin{figure}
        \begin{center}\adjustimage{max size={\linewidth}{0.4\paperheight}}{paper_files/paper_10_1.png}\end{center}
        \caption{Different formats used to reference institution}
        \label{figA}
    \end{figure}
...
be seen in \ref{figA} \ref{figB}. Hence, we first detect the structure
of the header section before extracting the association. We tag the
substring present in this section using a combination of \emph{named
entity detection}, using the implementation provided by \emph{spacy},
simple string comparison matching of the names of authors and
institutions we extracted from the metadata respectively from Approach 1
and tagging sections containing ``emails'' using regex , to detect where
in the header the email is. Then, taking the tagged section, we detect
which subsections correspond to which author-affiliation listing type,
and extract the author email - institution mapping accordingly.
...
be of 5. We obtain a graph of 10845 nodes and 148594 edges.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
Uploading 7403 kB. This may take a while{\ldots}
...
one are contained in the clusters found by method 2. See .
